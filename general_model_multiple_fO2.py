...
fO2_and_bulk_epsilon = []
fO2_and_silicate_epsilon = []
fO2_and_core_epsilon = []
...
for i in np.arange(-0.5, 1.0, 0.5):
    ...
    fO2_and_bulk_epsilon.append(
        (i, epsilon_182w_bulk))
    fO2_and_silicate_epsilon.append(
        (i, epsilon_182w_mantle))
    fO2_and_core_epsilon.append(
        (i, epsilon_182w_core))
...
for index, f in enumerate(fO2_and_bulk_epsilon):
    silicate = fO2_and_silicate_epsilon[index][1]
    core = fO2_and_core_epsilon[index][1]
    ax1_1.plot([i / (10**6) for i in time_range], f[1],
             color='black', label='IW {}'.format(f[0]))
    ax1_2.plot([i / (10**6) for i in time_range], silicate,
             color='green', label='IW {}'.format(f[0]))
    ax1_3.plot([i / (10**6) for i in time_range], core,
             color='blue', label='IW {}'.format(f[0]))
